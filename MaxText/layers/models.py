...
CodebookEmbed = embeddings.CodebookEmbed
Attention = attentions.Attention
RMSNorm = normalizations.RMSNorm
PositionalEmbedding = embeddings.PositionalEmbedding
CodebookPositionalEmbedding = embeddings.CodebookPositionalEmbedding
...
class CodebookDecoder(nn.Module):
  ...
  def get_decoder_layer(self):
    from layers import fbs_decoder_layer
    return fbs_decoder_layer.FBSDecoderLayer
  ...
  def __call__(
    ...
    logits_res = []
    if cfg.scan_layers:
        y, _ = self.scan_decoder_layers(cfg, BlockLayer, cfg.num_decoder_layers, "layers", mesh)(
            y,
            decoder_segment_ids,
            decoder_positions,
            deterministic,
...
class Transformer(nn.Module):
  ...
  def setup(self):
    ...
    cfg = self.config
    mesh = self.mesh
    self.shared_embedding = CodebookEmbed(
