...
class SyntheticDataIterator:
  ...
  def raw_generate_synthetic_data(config):
    ...
    output = {}
    output["inputs"] = jax.numpy.zeros((config.global_batch_size_to_load, config.max_target_length,config.codebook_dim+1), dtype=jax.numpy.int32)
    ...
    output["targets"] = jax.numpy.zeros((config.global_batch_size_to_load, config.max_target_length,config.codebook_dim+1), dtype=jax.numpy.int32)
    ...
    output["prompt_length"] = jax.numpy.ones(
        (config.global_batch_size_to_load, config.max_target_length), dtype=jax.numpy.int32
    )
    return output
