    ...
    for item in multihost_gen:
        ...
        if i%10240 == 0:
            ...
            if writer is not None:
                writer.close() 
            writer = ArrayRecordWriter(f"/bucket/dataset/hifi_tts_concat/hifi_tts_concat_train_part_{num}.arrayrecord", 'group_size:1')
        ...
        for k in range(GLOBAL_BATCH_SIZE):
            ...
            i+=1
            if previous_item_dict.get(speaker_id) is not None:
                tokens = jnp.stack((tokens,previous_item_dict[speaker_id]),axis=0)
                ...
                previous_item_dict[speaker_id] = tokens
    # except:
    #     writer.close()